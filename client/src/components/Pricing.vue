<template>
    <section class="pricing">
        <div class="pricing-overview">
            ...
            <div class="plans">
                <div v-for="plan in store.plans" :key="plan.id" :class="{plan: true, recommended: !activePlanId && matchingPlan.id === plan.id, active: activePlanId === plan.id}">
                    ...
                    <button v-if="activePlanId === plan.id" class="account" @click="$router.push('/account')">Review account</button>
            ...
            <div class="estimated-bill">
                ...
                <div class="estimate">
                    ...
                    <p>plan and use
                        <input type="number" v-model.number="estimatedRequests"> requests, your bill will be <span v-text="estimatedTotalCost"></span>. </p>
                </div>
            </div>
        </div>
    </section>
    ...
    export default {
        ...
        , computed: {
            ...
            estimatedTotalCost: function () {
                ...
                let extraRequests = this.estimatedRequests - this.matchingPlan.numRequests;
                if (extraRequests < 0) {
                    extraRequests = 0;
                }
                // Calculate the estimated total
                const total = 0.01 * extraRequests + store.getPlan(this.estimatedPlanId).rate;
...
<style scoped>
    ...
    .plan button.account {
        font-weight: 500;
        background: url('/assets/icons/arrow.svg') center right 17px no-repeat, #d782d9;
        padding-right: 50px;
    }
    
    .plan button.account:active {
        background: url('/assets/icons/arrow.svg') center right 17px no-repeat, #cf6bd2;
    }
    ...
    .estimate .select-plan {
        background: no-repeat right 10px center white url('/assets/icons/select-arrows.svg');
        border-radius: 4px;
        min-width: 120px;
        text-align: left;
    }
